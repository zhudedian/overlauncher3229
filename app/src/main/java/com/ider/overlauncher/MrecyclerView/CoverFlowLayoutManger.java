package com.ider.overlauncher.MrecyclerView;
...
//            cm.setSaturation(0.9f);
    ...
    public void onScrollStateChanged(int state) {
        ...
        Log.i("zzz", "onScrollStateChanged: state="+state);
        switch (state){
    ...
    public  void scrollToNextPosition(int next){
        if(next==1){//right scroll
            if (mSelectPosition < 0 || mSelectPosition > getItemCount() - 1) {
                scrollToPosition(0);
                return;
            }
            mOffsetAll = calculateOffsetForPosition(mSelectPosition+1);
            if (mRecycle == null || mState == null) {//如果RecyclerView还没初始化完，先记录下要滚动的位置
                mSelectPosition = mSelectPosition+1;
            } else {
                layoutItems(mRecycle, mState, SCROLL_RIGHT );
                onSelectedCallBack();
            }
        }
        else  if(next==2){//left
            ...
            if (mRecycle == null || mState == null) {//如果RecyclerView还没初始化完，先记录下要滚动的位置
                ...
                layoutItems(mRecycle, mState, SCROLL_LEFT );
                onSelectedCallBack();
            }
        }
    }
